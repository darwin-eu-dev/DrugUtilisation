test_that("inputs for addDailyDose function",{
    ...
    expect_true(all(resultT$daily_dose * resultT$quantity == 100*(as.integer(difftime(resultT$drug_exposure_end_date, resultT$drug_exposure_start_date, units="days")) + 1)))

    #test quantity
    resultT <- result %>% dplyr::filter(drug_dose_type %in% c("quantity") & drug_concept_id ==3)
    expect_true(all(resultT$daily_dose * resultT$quantity == 1*(as.integer(difftime(resultT$drug_exposure_end_date, resultT$drug_exposure_start_date, units="days")) + 1)))

    #test quantified
    resultT <- result %>% dplyr::filter(drug_dose_type %in% c("quantified") & drug_concept_id ==5)
    expect_true(all(resultT$daily_dose * resultT$quantity == 5*(as.integer(difftime(resultT$drug_exposure_end_date, resultT$drug_exposure_start_date, units="days")) + 1)))
