test_that("inputs for addDailyDose function",{
    ...
    expect_true(all(result$unit %in% c("milligram", "milliliter", "millicurie", "international unit", "milliequivalent", "millimole", NA)))

    compareNA <- function(v1,v2) {
      same <- (v1 - v2 < 0.000001) | (is.na(v1) & is.na(v2))
      same[is.na(same)] <- FALSE
      return(same)
    }

    expect_true(all(compareNA(result %>% dplyr::select(daily_dose) %>% pull(),
                          c(100, 100, 0.2, 600, 600, 600, 600, 600,
                            ...
                            42, 42, 100, 100, 105, 105, 105, 105, 25, 44,
                            7, 7, 3, 8, 1))))
