...
Indications will always be cohorts. An option that the package has is to create concept based cohorts using `CDMConnector::generateConceptCohortSet()`.
...
Currently you can evaluate the coverage of daily dose for a conceptList or overall using `summariseDoseCoverage`. You should restrict to a certain ingredient.
