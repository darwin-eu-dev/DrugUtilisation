...
To enrich your drug data, the DrugUtilisation package provides the `addRoute` function. This function utilizes an internal CSV file containing all possible routes for various drug dose forms supported by the package. You can access the CSV [here](https://github.com/darwin-eu-dev/DrugUtilisation/blob/main/data-raw/doseform_final.csv).
...
For detailed information about the patterns, their associated pattern ids, and combinations of `amount_unit`, `numerator_unit`, and `denominator_unit`, you can refer to the data:

- Names of patterns and their associated pattern ids can be found [here](https://github.com/darwin-eu-dev/DrugUtilisation/blob/main/data-raw/pattern_drug_strength.csv).
- Formulas and their corresponding formula ids, supported for different patterns, are available [here](https://github.com/darwin-eu-dev/DrugUtilisation/blob/main/data-raw/pattern_assessment_for_dose_final.csv).
