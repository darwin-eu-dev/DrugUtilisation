...
The parameter *durationRange* specifies the range within which the duration must fall. It should be a numeric vector of length two, with no NAs and the first value should be equal or smaller than the second one. It can ne set as: "eliminate", "median", "mean", "quantile25", "quantile75". The *dus_step2_0_inf* cohort in **STEP 2**.
...
Change the *summariseMode* parameter from AllEras to FirstEra and observe how it impacts the attrition of the *dus_step8_firstera cohort* in comparison to the *dus_alleras_step67* cohort. The number of records decreased at **STEP 8** because of the FirstEra summariseMode.
...
```{r message=FALSE, warning=FALSE}
cdm9 <- generateDrugUtilisationCohortSet(cdm,
...
cohortAttrition(cdm9$dus_step8_fixedtime) %>% select(number_records, reason, excluded_records, excluded_subjects)
```
