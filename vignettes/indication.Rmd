...
In this vignette, we demonstrate how to use the `addIndications()` function to establish a binary indicator between a drug utilisation cohort and another concept-based cohort. 
...
Use `indicationWindow` to specify the time window of interests, which are defined as the time window between the event and the indication. Additionally, you can use `unknownIndicationTable` to specify the tables to look for unknown indication. In above example we specified three different time window. So it added three columns to your cohort, one for each time window your specified.

## Summarise indications with summariseIndication()
To create a summary table of the indications cohort, you can use the `summariseIndication()` function. It contain similar arguments to  `addIndication()`, but it return a summary table of the indications.
...
Then you can also use the `tableIndication` function to create a `gt` style table from the output of `summariseIndication`.
