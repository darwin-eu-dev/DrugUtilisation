...
The first thing that we need is a `cdm_reference` object to our OMOP CDM instance.

- You can learn more about the *cdm reference* object in in omopgenerics: (`vignette("cdm_reference", package = "omopgenerics")`)[https://darwin-eu.github.io/omopgenerics/articles/cdm_reference.html].
- You can learn how to create cdm references using CDMConnector here: (`vignette("a04_DBI_connection_examples", package = "CDMConnector")`)[https://darwin-eu.github.io/CDMConnector/articles/a04_DBI_connection_examples.html].

The DrugUtilisation package contains some mock data that can be useful to test the package:
...
x |>
  dplyr::mutate(color = as.factor(c(0, 0, 0, 1))) |>
  doplot() + 
  ggplot2::ggtitle("washout + minObs")
```
