...
There are four possible forms of a conceptSet:
...
- A `codelist_with_details` object, see `vignette("codelists", package = "omopgenerics")`
```{r}
...
- **washout**: `requirePriorDrugWashout(daysff = 365)`
...
`generateDrugUtilisationCohortSet()` is a very specific function to create drug users cohorts. Most of the functions in this package need a cohort as input, this cohort will usually be a cohort generated using `generateDrugUtilisationCohortSet()`, but there exists some alternative to create cohorts
...
- You can create concept based cohorts using the `OHDSI` package `CohortConstructor`. See `vignette("a01_building_base_cohorts", package = "CohortConstructor")`.

- You can create cohorts based on ATLAS definitions using CDMConnector. See `vignette("a02_cohorts", package = "CDMConnector.")`.
